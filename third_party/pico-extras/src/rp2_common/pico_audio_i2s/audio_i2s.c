...
audio_i2s_setup(const audio_format_t *intended_audio_format,
  ...
  audio_i2s_program_init(audio_pio, sm, offset, config->data_pin,
...
static void pass_thru_producer_give(audio_connection_t *connection,
                                    audio_buffer_t *buffer) {
  queue_full_audio_buffer(connection->consumer_pool, buffer);
}

static void pass_thru_consumer_give(audio_connection_t *connection,
                                    audio_buffer_t *buffer) {
  queue_free_audio_buffer(connection->producer_pool, buffer);
}

static struct producer_pool_blocking_give_connection
    audio_i2s_pass_thru_connection = {
        .core = {
            .consumer_pool_take = consumer_pool_take_buffer_default,
...
bool audio_i2s_connect_s8(audio_buffer_pool_t *producer) {
  ...
  uint32_t system_clock_frequency = clock_get_hz(clk_sys);
  //    uint32_t divider = system_clock_frequency * 256 /
  //    producer->format->sample_freq * 16 * 4;
  uint32_t divider = system_clock_frequency * 4 /
                     producer->format->sample_freq; // avoid arithmetic overflow
  pio_sm_set_clkdiv_int_frac(audio_pio, shared_state.pio_sm, divider >> 8u,
                             divider & 0xffu);
...
void audio_i2s_set_enabled(bool enabled) {
  if (enabled != audio_enabled) {
    ...
    if (enabled) {
      ...
      // if there was a buffer in flight, it will not be freed by DMA IRQ, let's
      // do it manually
      if (shared_state.playing_buffer) {
        give_audio_buffer(audio_i2s_consumer, shared_state.playing_buffer);
        shared_state.playing_buffer = NULL;
